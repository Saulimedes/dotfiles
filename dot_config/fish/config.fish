...
set -gx PATH $PYENV_ROOT/shims $PYENV_ROOT/bin /usr/local/bin /usr/bin /usr/sbin /usr/local/sbin $PATH
...
if status --is-interactive
  ...
  function fish_user_key_bindings
    ...
    if type -q fzf-history-widget
      function __custom_fzf_history
        set -l original_fzf_tmux $FZF_TMUX
        if set -q TMUX
          set -gx FZF_TMUX 0
        end
        history --merge
        fzf-history-widget
        set -gx FZF_TMUX $original_fzf_tmux
      end
  ...
  if type -q pyenv
    pyenv init - | source
    if pyenv virtualenv-init - > /dev/null 2>&1
